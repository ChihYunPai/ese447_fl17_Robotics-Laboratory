...
typedef RT_MODEL_Read RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Read(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void Read_initialize();
   extern void Read_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Read_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Read:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Read -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
