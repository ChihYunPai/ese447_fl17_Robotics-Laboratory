...
typedef RT_MODEL_connect RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * connect(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void connect_initialize();
   extern void connect_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(connect_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://connect:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: connect -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
