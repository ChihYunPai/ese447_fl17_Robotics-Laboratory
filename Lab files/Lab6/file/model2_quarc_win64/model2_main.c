...
typedef RT_MODEL_model2 RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * model2(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void model2_initialize();
   extern void model2_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(model2_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://model2:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: model2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
